...
@section('page_title',    'Course Types')
@section('page_section',  'KERJA SAMA LUAR NEGERI')
@section('page_subtitle', 'Manage student course types')
...
<div class="sima-card">
    <div class="sima-card__header">
        <div>
            <h5 class="sima-card__title">Course Types</h5>
            <div class="sima-card__subtitle">Manage available course types for students</div>
        </div>
        <div>
            ...
            <tbody>
                ...
                <tr id="row-{{ $type->id }}">
                    ...
                    <td style="text-align:center;white-space:nowrap">
                        <button onclick="openEditModal({{ $type->id }}, '{{ addslashes($type->nama) }}')"
...
<div id="typeModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:1000;align-items:center;justify-content:center;">
    <div style="background:var(--c-surface);border-radius:var(--radius-lg);width:100%;max-width:440px;box-shadow:var(--shadow-lg);overflow:hidden;margin:16px;max-height:90vh;overflow-y:auto;">
        ...
        <div style="padding:20px 24px 16px;border-bottom:1px solid var(--c-border);display:flex;align-items:center;justify-content:space-between;">
            <div>
                <div id="modalTitle" style="font-size:15px;font-weight:700;color:var(--c-text-1)">Add Course Type</div>
                <div style="font-size:12px;color:var(--c-text-3);margin-top:2px">Fill in the type details below</div>
            </div>
        ...
        <form id="typeForm" style="padding:20px 24px;" onsubmit="submitForm(event)">
            ...
            <div style="margin-bottom:16px;">
                ...
                <input type="text" id="typeNama" name="nama" class="sima-input" placeholder="Enter course type name" required>
            </div>
...
function openAddModal() {
    editId = null;
    document.getElementById('modalTitle').textContent = 'Add Course Type';
    document.getElementById('typeNama').value = '';
    document.getElementById('formMethod').value = 'POST';
    document.getElementById('typeForm').action = '{{ route("kln.types.mahasiswa.store") }}';
    document.getElementById('typeModal').style.display = 'flex';
}

function openEditModal(id, nama) {
    editId = id;
    document.getElementById('modalTitle').textContent = 'Edit Course Type';
    document.getElementById('typeNama').value = nama;
    document.getElementById('formMethod').value = 'PATCH';
    document.getElementById('typeForm').action = '{{ route("kln.types.mahasiswa.update", ":id") }}'.replace(':id', id);
    document.getElementById('typeModal').style.display = 'flex';
}
...
function submitForm(e) {
    ...
    const nama  = document.getElementById('typeNama').value.trim();
...
function toggleActive(id, isActive) {
    ...
    fetch('{{ route("kln.types.mahasiswa.toggle", ":id") }}'.replace(':id', id), {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
